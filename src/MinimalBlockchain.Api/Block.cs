...
namespace MinimalBlockchain.Api
{
    internal record Block(int Index, DateTime Timestamp, List<Transaction> Transactions, int Proof, string PreviousHash)
    {
        ...
        {
            return $"Block {{ Index: {this.Index}, Timestamp: {this.Timestamp}, Proof: {this.Proof}, PreviousHash: {this.PreviousHash}, Transactions: [ {string.Join(", ", this.Transactions.Select(t => t.ToString()))} ] }}";
        }
    }
}